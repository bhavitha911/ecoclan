...
const Appliances = () => {
    ...
    useEffect(() => {
        axios.get('http://192.168.29.47:5000/api/products/3')
            .then((appliances) => {
                console.log("appliances.data", appliances.data);
                setAppliancesData(appliances.data);
                const initialPrices = {};
    ...
    const incrementPrice = (id) => {
        setPrices(prevPrices => ({
            ...prevPrices,
            [id]: (prevPrices[id] || 0) + 1 // If prevPrices[id] is undefined, default to 0 before incrementing
        }));
    };
